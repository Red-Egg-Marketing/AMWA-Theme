...
const apiUrl  = '/wp-json/AMWA/v2/programs';
const catUrl  = '/wp-json/wp/v2/product_cat';
...
const EditSelectedCaseStudies = ( { setAttributes, attributes, isSelected, clientId } ) => {
		...
		if ( (resources == undefined || resources.length == 0)) {
			...
			wp.apiFetch({
				url: apiUrl + '?post_types=product&ppp=15&category=program'
			}).then(resourcelist => {
								...
								<Fragment>
									...
									<div className="controls-wrapper">
										<div className="swiper-controls">
											<div className="fa swiper-button-prev"></div>
  											<div className="fa swiper-button-next"></div>
  										</div>
  									</div>
  								</Fragment>
  							)}
