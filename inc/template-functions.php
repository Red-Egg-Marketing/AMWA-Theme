 ...
 * @return array
 */
function amwa_theme_body_classes( $classes ) {
...
add_filter( 'body_class', 'amwa_theme_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function amwa_theme_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'amwa_theme_pingback_header' );


function amwa_theme_featured_image(int $id) {
	if ($id != 0) {
...
function amwa_theme_post_whitepaper_thumbnail(int $id) {
	if ($id != 0) {
...
function amwa_theme_posts_post_type(int $id) {
...
function amwa_theme_posts_topics_list(int $id, string $tax = 'category') {
...
function amwa_theme_posts_topics(int $id, string $tax = 'category', $label = '') {
	$topics = get_the_terms($id, $tax);
...
function amwa_theme_subscribe_custom_html(int $id = 1) {
...
function amwa_theme_post_form(int $id = 2) {
	if (function_exists('gravity_form')) {
...
function amwa_theme_check_cookie() {
	?>
...
function amwa_theme_get_asset($id) {
...
function amwa_theme_cookie_pick() {
...
add_action( 'wp_ajax_nopriv_amwa_theme_cookie_pick', 'amwa_theme_cookie_pick' );
add_action( 'wp_ajax_amwa_theme_cookie_pick', 'amwa_theme_cookie_pick' );

function amwa_theme_posts_pagination() {
...
function amwa_theme_embed_video(int $id) {
...
function amwa_theme_post_video(int $id) {
	if (function_exists('get_field')) {
		$video = get_field('featured_video', $id);
		$vid_src = $video['url'];
		$width = $video['width'];
...
function amwa_theme_new_excerpt_more($more) {
    return '...';
}
add_filter('excerpt_more', 'amwa_theme_new_excerpt_more');


function amwa_theme_excerpt_length( $length ) {
    return 24;
}
add_filter( 'excerpt_length', 'amwa_theme_excerpt_length', 999 );
...
function amwa_theme_featured_post() {
	...
	if ($query->have_posts()) {
		while($query->have_posts()) {
			$query->the_post();
			$id = get_the_ID();
			$post .= amwa_theme_resource_card($id, true, 'Read More', true, false);
		}
		wp_reset_postdata();
	}
