...
const template = [
	...
	['amwa-theme-block/shortcode-section'],
	['amwa-theme-block/content-column'],
];

const allowBlocks = ['amwa-theme-block/content-column', 'amwa-theme-block/shortcode-section', 'amwa-theme-block/header-intro'];

const EditPortraits = ( { attributes, setAttributes, clientId } ) => {
		const {
			media, title, image, bgColor, bgSlug, color, animateScroll, columnwidth, padding, margin, blockId
		} = attributes;

		const blockProps = useBlockProps({
			className: 'portraits' + ' ' + (bgSlug != '' ? ' ' + bgSlug + ' with-bg' : '')  + (animateScroll == true ? ' scroll-activate' : '') + (' ' + columnwidth),
		});
