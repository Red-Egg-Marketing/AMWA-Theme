...
function amwa_theme_return_taxonomies($post_types) {
...
function amwa_get_business_hours() {
	if (function_exists('get_field')) {
		...
		if (!empty($hour_group['days'])) {
			$days = $hour_group['days'];
			foreach($days as $day) {
				$d = $day['day'];
				$weekday = explode('|', $d);
				if (is_array($weekday)) {
		...
		return [$day_language, $hours];
	}
}

add_action( 'rest_api_init', function () {
  register_rest_route( 'AMWA/v2', '/hours/', 
  	[
    	'methods' => 'GET, POST',
    	'callback' => 'amwa_get_business_hours',
...
function amwa_theme_build_post_tax_array($posts, $tax) {
	if (sizeof($tax) > 0) {
		$len = sizeof($tax);
		$post_array = [];
		$tax_array = [];
...
function amwa_theme_return_resources() {
	$post_types = ['post'];
	...
	if ($query->have_posts()) {
		...
		$taxes = amwa_theme_return_taxonomies($post_types);

		$resources = amwa_theme_build_post_tax_array($result, $taxes);
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'AMWA/v2', '/resources/', 
  	[
    	'methods' => 'GET',
    	'callback' => 'amwa_theme_return_resources',
    	'permission_callback' => '__return_true'
  	] 
  );
 });
...
function amwa_theme_case_study_stat_block($id) {
	if ($id != null) {
...
function amwa_theme_return_posts($data) {
	...
	if ($query->have_posts()) {
		while($query->have_posts()) {
			...
			if ($html == false) {
				...
				$posts .= amwa_theme_resource_card($id);
			}
		}
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'AMWA/v2', '/posts/', 
  	[
    	'methods' => 'GET',
    	'callback' => 'amwa_theme_return_posts',
    	'permission_callback' => '__return_true'
  	] 
  );
 });

function amwa_theme_resource_card($id, $cats = true, $read = 'Read More', $image = true, $exc = true) {
	if ($id != null) {
		$permalink = get_the_permalink($id);
		$title = get_the_title($id);
		$excerpt = get_the_excerpt($id);
		// $terms = $cats == true ? amwa_theme_posts_topics_list($id, 'category') : amwa_theme_posts_post_type($id);
		$terms = $cats == true ? amwa_theme_posts_topics_list($id, 'category') : false;
		$thumbnail = get_the_post_thumbnail_url($id, 'post-landscape') != false ? get_the_post_thumbnail_url($id, 'post-landscape') : get_the_post_thumbnail_url($id, 'thumbnail');
