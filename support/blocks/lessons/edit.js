...
const EditLessons = ( { setAttributes, attributes, isSelected, clientId } ) => {
		...
		if ( (resources == undefined || resources.length == 0)) {
			...
			wp.apiFetch({
				url: apiUrl + '?post_types=lesson&ppp=15'
			}).then(resourcelist => {
		...
		const setCategoryPosts = (value) => {
			activateSwipe(false);
			wp.apiFetch({
				url: apiUrl + '?category=' + value + '&post_types=lesson&ppp=-1'
			}).then(resourcelist => {
								...
								<Fragment>
									<div className="swiper-wrapper">
											...
											{ tempSwip && tempResources.length == 0 && (
												<Fragment>
													<div className="error">
														<h3>There are no available lessons matching your filters. Please try something else.</h3>
													</div>
												</Fragment>
											)}
									</div>
